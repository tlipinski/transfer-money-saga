...
import net.tlipinski.sagas.bank.Balance.{BalanceResult, EmptyTransaction, SubAmountTooHigh, Transaction, TransactionAdded, TransactionExists, TransactionProcessed}
...
case class Balance(
    ...
    outbox: List[OutboxMessage[MessageOut]],
    processed: List[TransactionId]
) {
  def withNewOutbox(outboxMsg: OutboxMessage[MessageOut]): Balance = {
    this.copy(outbox = outboxMsg :: outbox)
  }
...
object Balance {
  def init(playerId: String, amount: Int): Balance =
    Balance(playerId, amount, List.empty, List.empty, List.empty)
  ...
  case class TransactionAdded(balance: Balance)      extends BalanceResult
  case class SubAmountTooHigh(balance: Balance)      extends BalanceResult
