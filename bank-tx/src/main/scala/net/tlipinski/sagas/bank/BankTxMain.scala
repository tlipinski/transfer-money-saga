...
import net.tlipinski.publisher.{DeadLetterPublisher, KafkaMessagePublisher, RecordHandler, RetryUntilDead}

object BankTxMain extends IOApp {
  ...
  override def run(args: List[String]): IO[ExitCode] = {
    (for {
      ...
      val balanceRepo = (ctx: AttemptContext) => new BalanceRepo(ctx, cluster)
      val outboxRepo = (ctx: AttemptContext) => new OutboxRepo(ctx, cluster)
