...
class BalanceTest extends AnyFlatSpec with should.Matchers {
  ...
  it should "process balance change" in {
    ...
    out1.topic shouldBe "reply"
    out1.message shouldBe a[BalanceChanged]

    val TransactionAdded(b2, out2) =
    ...
    out2.message shouldBe a[BalanceChanged]

    val SubAmountTooHigh(b3, out3) =
      Balance(
        playerId,
        1000,
    ...
    out3.message shouldBe a[BalanceNotChanged]

    val TransactionAdded(b4, out4) =
    ...
    out4.message shouldBe a[BalanceChanged]

    val SubAmountTooHigh(b5, out5) =
      Balance(
        playerId,
        1000,
        List(Transaction(TransactionId("x"), -1000)),
    ...
    out5.message shouldBe a[BalanceNotChanged]

    val SubAmountTooHigh(b6, out6) = Balance(
      playerId,
      1000,
    ...
    out6.message shouldBe a[BalanceNotChanged]
